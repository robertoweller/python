...
class IntSet:
    # Set onde vai ficar os inteiros
    vals = set()
    ...
    def inteiros(self, inte):
        # Se nao estiver na lista e for um numero inteiro
        if self.str_is_int(inte):
                
            self.vals.add(inte)
         

    def __str__(self):
        self.setIn = ', '.join(map(str, self.vals))
...
if __name__ == '__main__':
    ...
    # mas não adiciona na lista, se digita "s" sai do looping
    while per != "s":
        IntSet(per)
        per = input("Insira um valor (ou s para sair): ")
